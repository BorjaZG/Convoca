...
export async function listUsers(query: { page?: string; limit?: string; role?: string }) {
  ...
  const [users, total] = await prisma.$transaction([
    prisma.user.findMany({ where, skip, take: limit, select: SAFE_SELECT, orderBy: { createdAt: 'desc' } }),
...
export async function updateUserRole(id: string, data: UpdateRoleInput) {
  ...
  return prisma.user.update({ where: { id }, data: { role: data.role as Role }, select: SAFE_SELECT });
}
